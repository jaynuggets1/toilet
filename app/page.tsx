...
if (user) {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-6 text-white">
      {/* Changed bg-slate-900 to a transparent slate with blur */}
      <div className="w-full max-w-md bg-slate-900/40 border border-white/10 p-10 rounded-[2.5rem] shadow-2xl backdrop-blur-md">
        <div className="mb-10 text-center">
          {/* Made the icon container slightly transparent too */}
          <div className="w-16 h-16 bg-blue-600/80 rounded-2xl mx-auto mb-6 flex items-center justify-center text-3xl shadow-lg shadow-blue-500/20">
            ☁️
          </div>
          <h2 className="text-3xl font-bold tracking-tight">
            {isSignUp ? "Create Account" : "Welcome Back"}
          </h2>
          <p className="text-slate-300/80 mt-2 font-medium">
            {isSignUp ? "Start tracking the skies today." : "Log in to see your local weather."}
          </p>
        </div>
        ...
        <form onSubmit={isSignUp ? handleSignUp : handleLogin} className="space-y-4">
          {isSignUp && (
            <input 
              ...
              className="w-full p-4 bg-white/5 border border-white/10 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/50 transition-all placeholder:text-slate-500" 
          ...
          <input 
            ...
            required 
          />
          <button className="w-full py-4 bg-blue-600 hover:bg-blue-500 font-bold rounded-2xl transition-all shadow-lg shadow-blue-600/30">
